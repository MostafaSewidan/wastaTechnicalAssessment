...
class HomeController extends Controller
{
    ...
    public function yearlyBestSellingPrice()
    {
        ...
        ->leftjoin('variants','product_sales.variant_id', '=', 'variants.id')
        ->select(DB::raw('products.name as product_name, products.code as product_code, variants.name as variant_name, variants.image as product_images, sum(total) as total_price'))
        ->whereDate('product_sales.created_at', '>=' , date("Y").'-01-01')
        ->whereDate('product_sales.created_at', '<=' , date("Y").'-12-31')
        ->groupBy('product_sales.variant_id')
    ...
    public function yearlyBestSellingQty()
    {
        ...
        ->select(DB::raw('products.name as product_name, variants.name as variant_name , products.code as product_code, variants.image as product_images, sum(product_sales.qty) as sold_qty'))
        ->whereDate('product_sales.created_at', '>=' , date("Y").'-01-01')
        ->whereDate('product_sales.created_at', '<=' , date("Y").'-12-31')
        ->groupBy('product_sales.variant_id')
